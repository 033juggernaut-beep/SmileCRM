 ...
 * - Minimal robot/assistant icon (flat, outline style)
 ...
 * - No animation by default, no chatbot bubbles
 * - Same blue accent color as rest of UI
 */

import { useState } from 'react';
import { Mic, MessageSquare, X, Stethoscope, Calendar, Wallet, Megaphone } from 'lucide-react';
import type { FloatingAIAssistantProps, AIAssistantAction } from '../types';
  ...
  const capabilities = [
    {
      ...
      icon: Stethoscope,
      label: 'Диагноз',
      description: 'Заполнение диагноза',
    },
    {
      id: 'visit' as const,
      icon: Calendar,
      label: 'Визиты',
      description: 'Добавление визитов',
    },
    {
      id: 'finance' as const,
      icon: Wallet,
      label: 'Финансы',
      description: 'Обновление оплат',
    },
    {
      id: 'marketing' as const,
      icon: Megaphone,
      label: 'Маркетинг',
      description: 'Генерация текстов',
    },
  ];

  const handleCapabilityClick = (capId: 'diagnosis' | 'visit' | 'finance' | 'marketing') => {
    if (inputText.trim()) {
